...
export default function CardOwner({ property, currentUser, onClose, onIntentChange }: CardOwnerProps) {
    ...
    const handleSaveDetails = async () => {
        ...
        // 1. Update Property Label
        if (title !== property.display_label) {
             await supabase.from('properties').update({ display_label: title }).eq('id', property.id)
        }
        
        // 2. Update Story (summary_text) in home_story or properties?
        // Spec implies home_story table usually, but previously we saw summary_text on property.
        // Let's assume property.summary_text is what we want for now based on simplicity, 
        // OR fallback to handling home_story upsert.
        // Checking schema via property object: property.summary_text exists on MapProperty type usually.
        // SAFE BET: Update 'properties' summary_text if column exists, or home_story.
        // Let's try upserting home_story for richer content.
        ...
        >
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-8">
                    ...
                    <div className="space-y-3">
                        <div>
                            ...
                            <input 
                                ...
                                className="w-full p-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm font-semibold focus:ring-2 focus:ring-indigo-500 outline-none transition-shadow"
                            />
                        </div>
                        <div>
                ...
                <section className="space-y-3">
                    ...
                    >
                        {property.image_url ? (
                            <img src={property.image_url} alt="Main" className="w-full h-full object-cover" />
                        ) : (
